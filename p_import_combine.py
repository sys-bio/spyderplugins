...
class C2P(QWidget, SpyderPluginMixin):
...
def Translatecombine(combine):
        ...
        if __manifest == True:
            ...
            for child in root:
                ...
                if 'sbml' in formtype:
                    ...
                    sedmlloc = loc
                    sedmlloc = sedmlloc[1:]
            return (zipextloc, sbmlloc, sedmlloc)
        else:
    ...
    def translate(combine, filename):
        tempPath = [[]]
        zipextloc, sbmlloc, sedmlloc = manifestsearch(combine)
        sbml = te.readFromFile(zipextloc + sbmlloc)
        sbmlstr = te.sbmlToAntimony(sbml)
        sedmlstr = se.sedml_to_python(zipextloc + sedmlloc)
        rePath = r"(\w*).load\('(.*)'\)"
